...
export const serializeActionKey = ({
  ...
  transactionIndex,
  index,
}: ActionKey): Buffer => bytewise.encode([
  actionKeyPrefix,
  blockIndex,
  transactionIndex,
  index,
...
export const getActionKeyMin = ({
  ...
  indexStart,
}: ActionsKey): Buffer => bytewise.encode(bytewise.sorts.array.bound.lower([
  actionKeyPrefix,
  blockIndexStart,
  transactionIndexStart,
  indexStart,
...
export const getActionKeyMax = ({
  ...
  indexStop,
}: ActionsKey): Buffer => bytewise.encode(bytewise.sorts.array.bound.upper([
  actionKeyPrefix,
  blockIndexStop == null
...
const serializeValidatorKey = ({ publicKey }: ValidatorKey): Buffer =>
  bytewise.encode([
    validatorKeyPrefix,
    common.ecPointToBuffer(publicKey)
  ]);
...
const createSerializeUInt160Key = (prefix: string) =>
  (input: { +hash: UInt160 }): Buffer => bytewise.encode([
    prefix,
    serializeUInt160Key(input),
  ]);
const createSerializeUInt256Key = (prefix: string) =>
  (input: { +hash: UInt256 }): Buffer => bytewise.encode([
    prefix,
    serializeUInt256Key(input),
  ]);

const createSerializeUInt160KeyString = (prefix: string) =>
  (input: { +hash: UInt160 }): string =>
    `${prefix}:${common.uInt160ToString(input.hash)}`
const createSerializeUInt256KeyString = (prefix: string) =>
  (input: { +hash: UInt256 }): string =>
    `${prefix}:${common.uInt256ToString(input.hash)}`

export const accountMinKey = bytewise.encode(bytewise.sorts.array.bound.lower([
  accountKeyPrefix,
]));
export const accountMaxKey = bytewise.encode(bytewise.sorts.array.bound.upper([
  accountKeyPrefix,
]));

export const serializeOutputKeyString = ({ index, hash }: OutputKey): string =>
  `${common.uInt256ToString(hash)}:${index}`;
